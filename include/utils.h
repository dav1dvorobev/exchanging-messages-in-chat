...
void logging(const std::string& __message, const std::string& __path) {
    ...
    LOGFILE << '[' << std::setfill('0') << std::setw(2) << localTime->tm_mday << '-'
            << std::setfill('0') << std::setw(2) << localTime->tm_mon + 1 << '-'
            << localTime->tm_year + 1900 << ' ' << std::setfill('0') << std::setw(2)
            << localTime->tm_hour << ':' << std::setfill('0') << std::setw(2) << localTime->tm_min
            << ':' << std::setfill('0') << std::setw(2) << localTime->tm_sec << "] " << __message;
    LOGFILE.close();
    loggingMutex.unlock();
}
bool validation(const std::string& __buf) {
    if (__buf.size() > 64) { return false; }
    if (!(__buf.back() == '.' || __buf.back() == '!' || __buf.back() == '?')) { return false; }
    for (const char& iter : __buf) {
        if (static_cast<int>(iter) < 32) { return false; }
    }
    return true;
}
...
std::string setColor(std::string __buf, const std::vector<size_t>& __RGB = {0, 0, 0}) {
    size_t R = __RGB[0], G = __RGB[1], B = __RGB[2];
    std::string color =
        "\x1B[38;2;" + std::to_string(R) + ';' + std::to_string(G) + ';' + std::to_string(B) + 'm';
    return color + __buf + RESET;
}
std::string setColorRandom(std::string __buf) {
