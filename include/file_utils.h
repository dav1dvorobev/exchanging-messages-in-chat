...
void logging(const std::string& __message, const std::string& __path) {
    ...
    LOGFILE << '[' << std::setfill('0') << std::setw(2) << localTime->tm_mday
            ...
            << std::setw(2) << localTime->tm_hour << ':' << std::setfill('0')
            << std::setw(2) << localTime->tm_min << ':' << std::setfill('0')
            << std::setw(2) << localTime->tm_sec << "] " << __message;
    LOGFILE.close();
    loggingMutex.unlock();
}
bool validation(const std::string& __buf) {
    if (__buf.size() > 64) { return false; }
    if (!(__buf.back() == '.' || __buf.back() == '!' || __buf.back() == '?')) {
        return false;
    }
